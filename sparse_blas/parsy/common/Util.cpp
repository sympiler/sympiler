...
namespace sym_lib {
 namespace parsy {
  ...
  bool
  readMatrix_rect(std::string fName, size_t &n_row, size_t &n_col, size_t &NNZ, int *&col, int *&row, double *&val) {
   /*This function reads the input matrix from "fName" file and
    * allocate memory for matrix A, L and U.
    * - The input file is a coordinate version and e
    * ach row of the file shows (col, row, nnz)
...
/*  if(i==660)
  ...
  int
  zero_diag_removal(size_t n, size_t NNZ, const int *col, const int *row, double *val, double threshold,
                    double reg_diag) {
   int num_zeros = 0;
   for (int i = 0; i < n; ++i) {
