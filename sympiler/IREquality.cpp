...
/*        if (compare_scalar(a.ptr->type_info(), b.ptr->type_info()) != Equal) {
            return result;
        }*/ //FIXME seems run-time stuffs that we don't need now
...
//        compare_expr(e->index, op->index); //FIXME: make it for pinter type
    }
/*
...
//        compare_names(s->name, op->name);
        ...
        compare_expr(s->value, op->value);
  //      compare_expr(s->index, op->index); //FIXME change it for ponter type
    }
/*
...
*/ //FIXME: overloading the operators for the expressions
